...
The code tutorial [p2p programming in ethereumprogpow](https://github.com/nolash/ethereumprogpow-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `ethereumprogpow` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `ethereumprogpow` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
